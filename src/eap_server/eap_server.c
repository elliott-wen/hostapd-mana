...
#define EAP_MAX_AUTH_ROUNDS 500000 //MANA
		 ...
		 int phase2)
{
	...
	char ident = 't';
	char print_ident[1024];
	size_t copylength = 1023;
	if(identity_len<copylength) copylength = identity_len;
	os_memcpy(print_ident, identity, copylength);
	print_ident[copylength]='\0';
	wpa_printf(MSG_INFO, "MANA (EAP) : identity: %s", print_ident);
...
SM_STATE(EAP, DISABLED)
{
	...
	wpa_printf(MSG_INFO, "State: Disable");	
	sm->num_rounds = 0;
}
...
SM_STATE(EAP, INITIALIZE)
{
	...
	wpa_printf(MSG_INFO, "State: Initialize");
...
SM_STATE(EAP, PICK_UP_METHOD)
{
	...
	wpa_printf(MSG_INFO, "PICK UP METHOD");
	if (eap_sm_Policy_doPickUp(sm, sm->respMethod)) {
		...
		if (sm->m && sm->m->initPickUp) {
			...
			if (sm->eap_method_priv == NULL) {
				wpa_printf(MSG_INFO, "EAP: Failed to "
...
SM_STATE(EAP, RETRANSMIT)
{
	...
	wpa_printf(MSG_INFO, "RETRANSMIT");
	sm->retransCount++;
...
SM_STATE(EAP, SEND_REQUEST)
{
	...
	wpa_printf(MSG_INFO, "SEND_REQUEST");
	sm->retransCount = 0;
...
SM_STATE(EAP, INTEGRITY_CHECK)
{
	...
	//wpa_printf(MSG_INFO, "INTEGRITY_CHECK");
	if (!eap_hdr_len_valid(sm->eap_if.eapRespData, 1)) {
		sm->ignore = TRUE;
		wpa_printf(MSG_INFO, "INTEGRITY_CHECK FAILED");
		return;
	}
...
SM_STATE(EAP, METHOD_REQUEST)
{
	...
	wpa_printf(MSG_INFO, "METHOD_REQUEST");
	if (sm->m == NULL) {
		wpa_printf(MSG_INFO, "EAP: method not initialized");
		return;
	}

	sm->currentId = eap_sm_nextId(sm, sm->currentId);
	wpa_printf(MSG_INFO, "EAP: building EAP-Request: Identifier %d",
		   sm->currentId);
...
SM_STATE(EAP, METHOD_RESPONSE)
{
	...
	wpa_printf(MSG_INFO, "METHOD_RESPONSE");
	if (!eap_hdr_len_valid(sm->eap_if.eapRespData, 1))
		return;
...
SM_STATE(EAP, PROPOSE_METHOD)
{
	int vendor;
	EapType type;
	wpa_printf(MSG_INFO, "PROPOSE_METHOD");
...
try_another_method:
	...
	if (sm->m) {
		...
		if (sm->eap_method_priv == NULL) {
			wpa_printf(MSG_INFO, "EAP: Failed to initialize EAP "
	...
	if (sm->m == NULL) {
		wpa_printf(MSG_INFO, "EAP: Could not find suitable EAP method");
...
SM_STEP(EAP)
{
	if (sm->eap_if.eapRestart && sm->eap_if.portEnabled)
		...
		if (sm->num_rounds == EAP_MAX_AUTH_ROUNDS + 1) {
			wpa_printf(MSG_INFO, "EAP: more than %d "
	...
	case EAP_RECEIVED:
		if (sm->rxResp && (sm->respId == sm->currentId) &&
			...
			wpa_printf(MSG_INFO, "EAP: RECEIVED->DISCARD: "
				   ...
				   int methodTimeout)
{
	...
	if (methodTimeout) {
		 ...
		 */
		wpa_printf(MSG_INFO, "EAP: retransmit timeout %d seconds "
...
static void eap_sm_parseEapResp(struct eap_sm *sm, const struct wpabuf *resp)
{
	...
	if (resp == NULL || wpabuf_len(resp) < sizeof(*hdr)) {
		wpa_printf(MSG_INFO, "EAP: parseEapResp: invalid resp=%p "
			   "len=%lu", resp,
	...
	if (plen > wpabuf_len(resp)) {
		wpa_printf(MSG_INFO, "EAP: Ignored truncated EAP-Packet "
			   "(len=%lu plen=%lu)",
	...
	if (plen > sizeof(*hdr)) {
		...
		if (sm->respMethod == EAP_TYPE_EXPANDED) {
			if (plen < sizeof(*hdr) + 8) {
				wpa_printf(MSG_INFO, "EAP: Ignored truncated "
	...
	wpa_printf(MSG_INFO, "EAP: parseEapResp: rxResp=%d respId=%d "
...
static int eap_sm_getId(const struct wpabuf *data)
{
	...
	wpa_printf(MSG_INFO, "EAP: getId: id=%d", hdr->identifier);
...
static struct wpabuf * eap_sm_buildSuccess(struct eap_sm *sm, u8 id)
{
	struct wpabuf *msg;
	struct eap_hdr *resp;
	wpa_printf(MSG_INFO, "EAP: Building EAP-Success (id=%d)", id);
...
static struct wpabuf * eap_sm_buildFailure(struct eap_sm *sm, u8 id)
{
	struct wpabuf *msg;
	struct eap_hdr *resp;
	wpa_printf(MSG_INFO, "EAP: Building EAP-Failure (id=%d)", id);
				 ...
				 size_t len)
{
	...
	if (sm->user->phase2) {
		wpa_printf(MSG_INFO, "EAP: EAP-Nak received after Phase2 user"
...
static EapType eap_sm_Policy_getNextMethod(struct eap_sm *sm, int *vendor)
{
	...
	wpa_printf(MSG_INFO, "EAP: getNextMethod: vendor %d type %d",
		   *vendor, next);
	return next;
}
...
static int eap_sm_Policy_getDecision(struct eap_sm *sm)
{
	if (!sm->eap_server && sm->identity && !sm->start_reauth) {
		wpa_printf(MSG_INFO, "EAP: getDecision: -> PASSTHROUGH");
	...
	if (sm->m && sm->currentMethod != EAP_TYPE_IDENTITY &&
		...
		wpa_printf(MSG_INFO, "EAP: getDecision: method succeeded -> "
			   "SUCCESS");
		sm->update_user = TRUE;
	...
	if (sm->m && sm->m->isDone(sm, sm->eap_method_priv) &&
	    !sm->m->isSuccess(sm, sm->eap_method_priv)) {
		wpa_printf(MSG_INFO, "EAP: getDecision: method failed -> "
			   "FAILURE");
		sm->update_user = TRUE;
	...
	if ((sm->user == NULL || sm->update_user) && sm->identity &&
		...
		if (eap_user_get(sm, sm->identity, sm->identity_len, 0) != 0) {
			wpa_printf(MSG_INFO, "EAP: getDecision: user not "
		...
		if (id_req && sm->user &&
		    sm->user->methods[0].vendor == EAP_VENDOR_IETF &&
		    sm->user->methods[0].method == EAP_TYPE_IDENTITY) {
			wpa_printf(MSG_INFO, "EAP: getDecision: stop "
	...
	if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS &&
	    (sm->user->methods[sm->user_eap_method_index].vendor !=
	     ...
	     EAP_TYPE_NONE)) {
		wpa_printf(MSG_INFO, "EAP: getDecision: another method "
			   "available -> CONTINUE");
		return DECISION_CONTINUE;
	}

	if (sm->identity == NULL || sm->currentId == -1) {
		wpa_printf(MSG_INFO, "EAP: getDecision: no identity known "
			   "yet -> CONTINUE");
		return DECISION_CONTINUE;
	}

	wpa_printf(MSG_INFO, "EAP: getDecision: no more methods available -> "
		   "FAILURE");
	return DECISION_FAILURE;
}
				   ...
				   struct eap_config *conf)
{
	...
	wpa_printf(MSG_INFO, "EAP: Server state machine created");
...
void eap_server_sm_deinit(struct eap_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_printf(MSG_INFO, "EAP: Server state machine removed");
	if (sm->m && sm->eap_method_priv)
		sm->m->reset(sm, sm->eap_method_priv);
	wpabuf_free(sm->eap_if.eapReqData);
	bin_clear_free(sm->eap_if.eapKeyData, sm->eap_if.eapKeyDataLen);
...
void eap_sm_pending_cb(struct eap_sm *sm)
{
	if (sm == NULL)
		return;
	wpa_printf(MSG_INFO, "EAP: Callback for pending request received");
	if (sm->method_pending == METHOD_PENDING_WAIT)
