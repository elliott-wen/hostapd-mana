...
{
	wpa_printf(MSG_INFO, "EAP-SIM: %s -> %s",
					   ...
					   struct eap_sim_data *data, u8 id)
{
	...
	wpa_printf(MSG_INFO, "EAP-SIM: Generating Start");
	...
	if (data->start_round == 1) {
		 ...
		 */
		wpa_printf(MSG_INFO, "   AT_ANY_ID_REQ");
		...
		wpa_printf(MSG_INFO, "   AT_FULLAUTH_ID_REQ");
		eap_sim_msg_add(msg, EAP_SIM_AT_FULLAUTH_ID_REQ, 0, NULL, 0);
	} else {
		wpa_printf(MSG_INFO, "   AT_PERMANENT_ID_REQ");
		eap_sim_msg_add(msg, EAP_SIM_AT_PERMANENT_ID_REQ, 0, NULL, 0);
	}
	wpa_printf(MSG_INFO, "   AT_VERSION_LIST");
	ver[0] = 0;
			      ...
			      const u8 *nonce_s)
{
	...
	if (data->counter <= EAP_SIM_MAX_FAST_REAUTHS) {
		...
		wpa_printf(MSG_INFO, "EAP-SIM: Max fast re-authentication "
	...
	wpa_printf(MSG_INFO, "   AT_IV");
	wpa_printf(MSG_INFO, "   AT_ENCR_DATA");
	eap_sim_msg_add_encr_start(msg, EAP_SIM_AT_IV, EAP_SIM_AT_ENCR_DATA);

	if (counter > 0) {
		wpa_printf(MSG_INFO, "   *AT_COUNTER (%u)", counter);
		eap_sim_msg_add(msg, EAP_SIM_AT_COUNTER, counter, NULL, 0);
	}

	if (nonce_s) {
		wpa_printf(MSG_INFO, "   *AT_NONCE_S");
		eap_sim_msg_add(msg, EAP_SIM_AT_NONCE_S, 0, nonce_s,
				EAP_SIM_NONCE_S_LEN);
	}

	if (data->next_pseudonym) {
		wpa_printf(MSG_INFO, "   *AT_NEXT_PSEUDONYM (%s)",
			   data->next_pseudonym);
	...
	if (data->next_reauth_id) {
		wpa_printf(MSG_INFO, "   *AT_NEXT_REAUTH_ID (%s)",
			   data->next_reauth_id);
					       ...
					       u8 id)
{
	...
	wpa_printf(MSG_INFO, "EAP-SIM: Generating Challenge");
	msg = eap_sim_msg_init(EAP_CODE_REQUEST, id, EAP_TYPE_SIM,
			       EAP_SIM_SUBTYPE_CHALLENGE);
	wpa_printf(MSG_INFO, "   AT_RAND");
	...
	if (sm->eap_sim_aka_result_ind) {
		wpa_printf(MSG_INFO, "   AT_RESULT_IND");
		eap_sim_msg_add(msg, EAP_SIM_AT_RESULT_IND, 0, NULL, 0);
	}

	wpa_printf(MSG_INFO, "   AT_MAC");
					    ...
					    struct eap_sim_data *data, u8 id)
{
	...
	wpa_printf(MSG_INFO, "EAP-SIM: Generating Re-authentication");
						  ...
						  u8 id)
{
	...
	wpa_printf(MSG_INFO, "EAP-SIM: Generating Notification");
	msg = eap_sim_msg_init(EAP_CODE_REQUEST, id, EAP_TYPE_SIM,
			       EAP_SIM_SUBTYPE_NOTIFICATION);
	wpa_printf(MSG_INFO, "   AT_NOTIFICATION (%d)", data->notification);
	...
	if (data->use_result_ind) {
		if (data->reauth) {
			...
			wpa_printf(MSG_INFO, "   *AT_COUNTER (%u)",
				   data->counter);
...
static struct wpabuf * eap_sim_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	default:
		wpa_printf(MSG_INFO, "EAP-SIM: Unknown state %d in "
			   "buildReq", data->state);
		break;
	}
	return NULL;
				  ...
				  struct eap_sim_attrs *attr)
{
	...
	wpa_printf(MSG_INFO, "EAP-SIM: Receive start response");
	...
	if (!attr->identity || attr->identity_len == 0) {
		wpa_printf(MSG_INFO, "EAP-SIM: Peer did not provide any "
			   "identity");
		goto failed;
	}
	...
	wpa_hexdump_ascii(MSG_INFO, "EAP-SIM: Identity",
	...
	if (username[0] == EAP_SIM_REAUTH_ID_PREFIX) {
		wpa_printf(MSG_INFO, "EAP-SIM: Reauth username '%s'",
			   username);
		data->reauth = eap_sim_db_get_reauth_entry(
			sm->eap_sim_db_priv, username);
		os_free(username);
		if (data->reauth == NULL) {
			wpa_printf(MSG_INFO, "EAP-SIM: Unknown reauth "
				   "identity - request full auth identity");
			/* Remain in START state for another round */
			return;
		}
		wpa_printf(MSG_INFO, "EAP-SIM: Using fast re-authentication");
	...
	if (username[0] == EAP_SIM_PSEUDONYM_PREFIX) {
		...
		wpa_printf(MSG_INFO, "EAP-SIM: Pseudonym username '%s'",
			   username);
		permanent = eap_sim_db_get_permanent(
			sm->eap_sim_db_priv, username);
		os_free(username);
		if (permanent == NULL) {
			wpa_printf(MSG_INFO, "EAP-SIM: Unknown pseudonym "
		...
		wpa_printf(MSG_INFO, "EAP-SIM: Permanent username '%s'",
			   username);
		//
		...
		wpa_printf(MSG_INFO, "EAP-SIM: Unrecognized username '%s'",
			   username);
		os_free(username);
		goto failed;
	}
...
skip_id_update:
	...
	if (attr->nonce_mt == NULL || attr->selected_version < 0) {
		wpa_printf(MSG_INFO, "EAP-SIM: Start/Response missing "
			   "required attributes");
		goto failed;
	}

	if (!eap_sim_supported_ver(data, attr->selected_version)) {
		wpa_printf(MSG_INFO, "EAP-SIM: Peer selected unsupported "
	...
	data->reauth = NULL;
	char tmpUsername[]="1000000000000000";
	data->num_chal = eap_sim_db_get_gsm_triplets(
		sm->eap_sim_db_priv, tmpUsername, EAP_SIM_MAX_CHAL,
		(u8 *) data->rand, (u8 *) data->kc, (u8 *) data->sres, sm);
	if (data->num_chal == EAP_SIM_DB_PENDING) {
		wpa_printf(MSG_INFO, "EAP-SIM: GSM authentication triplets "
	...
	while (identity_len > 0 && sm->identity[identity_len - 1] == '\0') {
		wpa_printf(MSG_INFO, "EAP-SIM: Workaround - drop last null "
			   "character from identity");
		identity_len--;
	}
	wpa_hexdump_ascii(MSG_INFO, "EAP-SIM: Identity for MK derivation",
				      ...
				      struct eap_sim_attrs *attr)
{
	...
	wpa_printf(MSG_INFO, "EAP-SIM: Challenge response includes the "
		   "correct AT_MAC");
				   ...
				   struct eap_sim_attrs *attr)
{
	...
	wpa_printf(MSG_INFO, "EAP-SIM: Re-authentication response includes "
		   "the correct AT_MAC");

	if (eattr.counter_too_small) {
		wpa_printf(MSG_INFO, "EAP-AKA: Re-authentication response "
...
static void eap_sim_process_client_error(struct eap_sm *sm,
					 ...
					 struct eap_sim_attrs *attr)
{
	wpa_printf(MSG_INFO, "EAP-SIM: Client reported error %d",
...
static void eap_sim_process_notification(struct eap_sm *sm,
					 ...
					 struct eap_sim_attrs *attr)
{
	wpa_printf(MSG_INFO, "EAP-SIM: Client replied to notification");
	if (data->notification == EAP_SIM_SUCCESS && data->use_result_ind)
		eap_sim_state(data, SUCCESS);
	else
		eap_sim_state(data, FAILURE);
			    ...
			    struct wpabuf *respData)
{
	...
	if (eap_sim_unexpected_subtype(data, subtype)) {
		wpa_printf(MSG_INFO, "EAP-SIM: Unrecognized or unexpected "
	...
	if (eap_sim_parse_attr(pos, end, &attr, 0, 0)) {
		wpa_printf(MSG_INFO, "EAP-SIM: Failed to parse attributes");
		if (subtype != EAP_SIM_SUBTYPE_CLIENT_ERROR &&
