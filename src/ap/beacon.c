		      ...
		      int ssi_signal)
{
	...
	if ((!elems.ssid || !elems.supp_rates)) {
		...
		if(probereq_udp_socket>0)
		{
			sprintf(send_buf,"%02x:%02x:%02x:%02x:%02x:%02x %s %d", MAC2STR(mgmt->sa),
		   	elems.ssid_len == 0 ? "broadcast" : wpa_ssid_txt(elems.ssid, elems.ssid_len),ssi_signal);
			sendto(probereq_udp_socket,send_buf,strlen(send_buf),0,(struct sockaddr *)&addr_client,sizeof(addr_client));
		}
#ifdef CONFIG_P2P
	...
	if (res == EXACT_SSID_MATCH) { //Probed for configured address
		...
		if (hapd->iconf->enable_karma) {
			...
			if (first_init == 1)
			{
				...
				char essid[256];
				int x;
				list = fopen("ssids.txt", "r");
				if(list == NULL)
				{
					wpa_printf(MSG_INFO, "MANA not preset ssids found, skip.");
				}
				else
				{
			...
			if (d == NULL) {
				wpa_printf(MSG_INFO, "MANA - Database:%d Adding SSID %s(%d) for STA " MACSTR " to the hash.", HASH_COUNT(karma_data), wpa_ssid_txt(elems.ssid, elems.ssid_len), elems.ssid_len, MAC2STR(mgmt->sa));
				d = (struct karma_ssid*)os_malloc(sizeof(struct karma_ssid));
